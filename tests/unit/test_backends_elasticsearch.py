"""Storage backend abstraction plan, Phase 5: unit tests for the
Elasticsearch ``KnowledgeBackend`` adapter, against ``FakeElasticsearch``
(an in-memory double implementing just the ``elasticsearch`` client
surface this adapter calls -- see ``_fake_elasticsearch.py``). No real
cluster, no ``elasticsearch`` import required for these tests:
``ElasticsearchKnowledgeBackend`` accepts a pre-built ``client=``
override precisely so tests (and any future caller with its own client)
can bypass ``elasticsearch_client.build_client``.
"""
...
from ragmonk.backends.elasticsearch_bulk import (
    BulkAction,
    BulkIndexError,
    run_bulk,
    run_bulk_or_raise,
...
def _config(**overrides: object) -> ServerStorageConfig:
    return ServerStorageConfig(engine="elasticsearch", url="http://fake:9200", **overrides)  # type: ignore[arg-type]


def _backend(
    client: FakeElasticsearch | None = None, **overrides: object
) -> ElasticsearchKnowledgeBackend:
    fake = client or FakeElasticsearch()
    backend = ElasticsearchKnowledgeBackend(_config(**overrides), client=fake)
    return backend
...
def test_health_true_when_reachable() -> None:
    backend = _backend(FakeElasticsearch(reachable=True))
    assert backend.health() is True


def test_health_false_when_unreachable() -> None:
    backend = _backend(FakeElasticsearch(reachable=False))
    assert backend.health() is False


def test_unreachable_cluster_raises_not_empty_results() -> None:
    """No silent local fallback: an unreachable Elasticsearch backend
    must raise on a real operation, never quietly return an empty/local
...
def test_describe_returns_engine_name_and_version() -> None:
    backend = _backend(FakeElasticsearch(version="8.11.0"))
    name, version = backend.describe()
    assert version == "8.11.0"
    assert "elastic" in name.lower() or name == "default"


def test_unsupported_old_version_raises_version_error() -> None:
    backend = _backend(FakeElasticsearch(version="7.17.0"))
    with pytest.raises(ElasticsearchVersionError):
        backend.describe()


def test_unsupported_old_version_health_is_false() -> None:
    backend = _backend(FakeElasticsearch(version="6.8.0"))
    assert backend.health() is False


def test_unparseable_version_raises_version_error() -> None:
    backend = _backend(FakeElasticsearch(version="not-a-version"))
    with pytest.raises(ElasticsearchVersionError):
        backend.describe()


def test_supported_version_boundary_is_accepted() -> None:
    backend = _backend(FakeElasticsearch(version="8.0.0"))
    name, version = backend.describe()
...
def test_bulk_batches_by_max_bytes() -> None:
    fake = FakeElasticsearch()
    config = BulkConfig(max_actions=1000, max_bytes=1, concurrency=1, max_retries=0)
    actions = [
        BulkAction(op="index", index="idx", doc_id=f"d{i}", source={"n": i}) for i in range(4)
    ]
    result = run_bulk(fake, actions, config)
    assert result.succeeded == 4
    # max_bytes=1 forces one action per batch.
    assert len(fake.bulk_calls) == 4


def test_bulk_retries_transient_failures_and_succeeds() -> None:
    fake = FakeElasticsearch(fail_ids={"d0": 2})
...
def test_bulk_surfaces_failure_once_retries_exhausted() -> None:
    fake = FakeElasticsearch(fail_ids={"d0": 100})
...
def test_bulk_partial_failure_does_not_swallow_successes() -> None:
    fake = FakeElasticsearch(fail_ids={"bad": 100})
    config = BulkConfig(max_actions=10, max_bytes=10_000_000, concurrency=1, max_retries=0)
    actions = [
...
def test_bulk_concurrent_batches_all_succeed() -> None:
    fake = FakeElasticsearch()
    config = BulkConfig(max_actions=2, max_bytes=10_000_000, concurrency=4, max_retries=0)
    actions = [
        BulkAction(op="index", index="idx", doc_id=f"d{i}", source={"n": i}) for i in range(9)
    ]
    result = run_bulk(fake, actions, config)
...
def test_publish_code_clear_only_removes_previous_generation() -> None:
    ...
    assert backend.get_entities_for_files(["f1"]) != []
    # Completion plan F6: an incremental re-publish writes into the
    # *same* (published) generation, and its delete half is scoped to
    # that generation -- see test_publish_code_other_generation_clear_keeps_published.
    backend.publish_code(PreparedCode(file_id="f1", source_id="s1", generation=0, clear_only=True))
    assert backend.get_entities_for_files(["f1"]) == []


def test_publish_code_other_generation_clear_keeps_published() -> None:
    """Completion plan F6: a rebuild writing generation N+1 must never
    delete generation N's (still published) entities for the same file.
    """
    backend = _backend()
    backend.publish_code(
        PreparedCode(file_id="f1", source_id="s1", generation=0, entities=[_entity()])
...
def test_publish_document_delete_only_clears_previous_generation() -> None:
    ...
    backend.publish_document(
        PreparedDocument(file_id="f1", source_id="s1", generation=0, delete_only=True)
    )
    assert backend.get_document_units_for_files(["f1"]) == []


# -- publish_embeddings / semantic_search -------------------------------------
...
def test_publish_embeddings_adds_dense_vector_field_once() -> None:
    fake = FakeElasticsearch()
    backend = _backend(fake)
    backend.ensure_schema()
    backend.publish_code(
    ...
    mapping = fake.indices.get_mapping(index="ragmonk-content")
    props = mapping["ragmonk-content"]["mappings"]["properties"]
...
def test_semantic_search_returns_hits_with_embedding() -> None:
    ...
    hits = backend.semantic_search([0.1, 0.2, 0.3], limit=5)
    assert any(hit.payload.get("entity_id") == "e1" for hit in hits)
