"""Storage backend abstraction plan follow-up: proves the OpenSearch
...
adds a fix for (see ``opensearch.py``'s ``_generation_filter_clause``).
"""
...
def _config() -> ServerStorageConfig:
    return ServerStorageConfig(engine="opensearch", url="http://fake:9200")


def _backend(fake: FakeOpenSearch) -> OpenSearchKnowledgeBackend:
    return OpenSearchKnowledgeBackend(_config(), client=fake)
