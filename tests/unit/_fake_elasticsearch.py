"""An in-memory fake standing in for ``elasticsearch.Elasticsearch`` in
unit tests -- implements just the surface
``ElasticsearchKnowledgeBackend`` calls (``info``/``cluster.health``/
``indices.*``/``get``/``index``/``delete``/``delete_by_query``/
``exists``/``bulk``/``search``/``count``), enough to exercise real
request/response shapes without needing a live cluster or even
``elasticsearch`` installed (this module has no such import).

Method signatures here use the same keyword-argument conventions this
adapter calls with (``query=``/``document=``/``properties=``/
``operations=`` rather than a single ``body=`` dict) -- the modern
``elasticsearch`` 8.x client's own calling convention, distinct from
``opensearchpy``'s ``body=``-based one (see ``_fake_opensearch.py``).

``FakeElasticsearch`` optionally injects failures for a bounded number
of attempts per document id, to exercise ``elasticsearch_bulk``'s retry
path.
"""
...
class NotFoundError(Exception):
    """Stands in for ``elasticsearch.NotFoundError`` -- named identically
    so the adapter's ``type(exc).__name__ != "NotFoundError"`` check
    (which avoids importing the real exception class, keeping this fake
    dependency-free) matches it the same way it would the real one.
    """
...
class _Indices:
    ...
    def create(
        ...
        self._mappings[index] = copy.deepcopy(mappings or {})
    ...
    def put_mapping(self, index: str, properties: dict[str, Any] | None = None) -> dict[str, Any]:
...
class FakeElasticsearch:
    ...
    succeeding (simulates transient 503s). ``version``: the cluster
    version this fake reports from ``info()``, for exercising the
    ...
    def __init__(
        ...
        *,
        reachable: bool = True,
        fail_ids: dict[str, int] | None = None,
        version: str = "8.11.0",
    ) -> None:
    ...
    def index(self, index: str, id: str, document: dict[str, Any]) -> dict[str, Any]:  # noqa: A002
        self.store.setdefault(index, {})[id] = copy.deepcopy(document)
        return {"_index": index, "_id": id, "result": "created"}

    def delete(self, index: str, id: str) -> dict[str, Any]:  # noqa: A002
        # Mirrors the real `elasticsearch` client: deleting a missing
        # document raises rather than silently no-op'ing (there is no
        # `ignore=[404]` kwarg on `delete`, unlike opensearchpy) --
        # `ElasticsearchKnowledgeBackend.delete_file` catches this by name.
        if id not in self.store.get(index, {}):
            raise NotFoundError(f"not found: {index}/{id}")
    ...
    def count(self, index: str, query: dict[str, Any]) -> dict[str, Any]:
        docs = self.store.get(index, {})
        matched = sum(1 for source in docs.values() if _matches(query, source))
    ...
    def search(
        ...
        docs = self.store.get(index, {})
        if knn is not None:
            # A minimal fake kNN: matches every doc carrying an
            # `embedding` field (and any `knn["filter"]` clauses), sorted
            # is not attempted -- good enough to exercise the code path,
            # not real ANN behavior.
            filter_query = knn.get("filter")
            matched = [
                {"_index": index, "_id": doc_id, "_score": 1.0, "_source": source}
                for doc_id, source in docs.items()
                if "embedding" in source
                and (filter_query is None or _matches(filter_query, source))
            ]
            return {"hits": {"hits": matched[:size], "total": {"value": len(matched)}}}
        effective_query = query or {"match_all": {}}
        matched = [
            {"_index": index, "_id": doc_id, "_score": 1.0, "_source": source}
            for doc_id, source in docs.items()
    ...
    def bulk(self, operations: list[dict[str, Any]]) -> dict[str, Any]:
        self.bulk_calls.append(copy.deepcopy(operations))
        items = []
        i = 0
