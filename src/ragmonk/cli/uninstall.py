"""``ragmonk uninstall [--keep-data] [--yes] [--json]``.

Removes the installed RagMonk application and, by default, all of its
data (``RAGMONK_HOME``) too -- pass ``--keep-data`` to remove only the
...
def _app_description(plan: uninstall_ops.UninstallPlan) -> str:
    ...
    return f"the ragmonk package (`{plan.method.value} uninstall`)"


@cli_command
def uninstall(
    keep_data: Annotated[
        bool, typer.Option("--keep-data", help="Remove only the application; keep RAGMONK_HOME.")
    ] = False,
    yes: Annotated[
    ...
    # *inside* RAGMONK_HOME (they share the same default root), so
