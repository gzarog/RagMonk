"""``ragmonk update [check|status|install]``.

``ragmonk update`` alone behaves like ``ragmonk update check`` -- the
single most useful thing to do with no arguments, mirroring tools like
``brew outdated``/``npm outdated``.

Distinct from ``ragmonk upgrade`` (``cli/upgrade.py``), which applies
database *schema* migrations to the already-installed RagMonk: these
commands discover and install a newer RagMonk *release* itself. See
...
from ragmonk.core.errors import EXIT_HEALTH_CHECK_FAILURE, RagMonkError
from ragmonk.update import cache, checker, installer, versioning
from ragmonk.update.models import UpdateCache

from ._common import cli_command, console, print_json

app = typer.Typer(help="Check for and install RagMonk updates.")
...
def check(json_output: Annotated[bool, typer.Option("--json")] = False) -> None:
    ...
    try:
        ...
        raise RagMonkError(f"could not check for updates: {exc}") from exc
    ...
    if update_available:
        ...
        console.print()
        console.print("    ragmonk update install")
    else:
        console.print(f"RagMonk {installed} is up to date.")
...
def status(json_output: Annotated[bool, typer.Option("--json")] = False) -> None:
    ...
    if cached is None:
        if json_output:
            ...
            return
        console.print(f"Installed: {installed}")
        console.print("Latest:    unknown -- run `ragmonk update check`")
        return
...
def install(json_output: Annotated[bool, typer.Option("--json")] = False) -> None:
    ...
    try:
        ...
        raise RagMonkError(str(exc)) from exc
    ...
    if json_output:
        ...
        console.print(f"RagMonk {outcome.installed_version} is already up to date.")
    else:
        console.print(f"[green]✓[/green] Installed RagMonk {outcome.installed_version}")
        if outcome.migrations_applied:
            ...
            console.print(
                ...
                "run `ragmonk upgrade` to retry"
            )
        if outcome.healthy:
            console.print("[green]✓[/green] Health check passed")
        else:
            console.print(
                "[yellow]![/yellow] Health check reported issues -- see `ragmonk doctor`"
            )
        console.print(f"\nRagMonk {outcome.installed_version} is ready.")
