"""``ragmonk install-agent [--write PATH] [--client NAME ...]``.
...
- No ``--client``: unchanged from the original, deliberately modest and
  safe design -- PRINTS the standard ``{"mcpServers": {"ragmonk": {...}}}``
...
- ``--client NAME`` (repeatable, or ``all``): automatically registers
  ...
  location for that specific client), and only the single ``ragmonk``
...
_JSON_ENTRY: dict[str, object] = {"command": "ragmonk", "args": ["serve", "--mcp"]}
...
def _merge_json_client(path: Path, *, top_level_key: str, include_type: bool) -> str:
    """Merges just ``<top_level_key>.ragmonk`` into ``path``'s existing
    ...
    ``ragmonk`` entry is corrected in place rather than left alone --
    ...
    if not isinstance(section, dict):
        section = {}
    if section.get("ragmonk") == entry:
        return f"already configured ({path})"
    was_present = "ragmonk" in section
    section["ragmonk"] = entry
...
def _merge_toml_client(path: Path) -> str:
    ...
    ``[mcp_servers.ragmonk]`` is appended as raw text, only when that
    ...
    ``ragmonk`` entry that doesn't already match is left alone and
    ...
    if path.is_file():
        try:
            ...
            return f"skipped ({path}: could not read/parse existing file: {exc})"
        existing = data.get("mcp_servers", {}).get("ragmonk")
        if existing == {"command": "ragmonk", "args": ["serve", "--mcp"]}:
            return f"already configured ({path})"
        if existing is not None:
            return (
                f"skipped ({path}: a different [mcp_servers.ragmonk] entry "
...
def install_agent(
    ...
    client: Annotated[
        ...
        typer.Option(
            ...
            help=(
                ...
                "vscode). Only the 'ragmonk' entry in that file is added "
    ...
    if client:
        ...
        for target in targets:
            ...
            # literal "[mcp_servers.ragmonk]" -- escaped so Rich's markup
    ...
    # JSON and nothing else -- pipeable (``ragmonk install-agent > x.json``)
