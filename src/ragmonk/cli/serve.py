"""``ragmonk serve --mcp`` -- start the stdio MCP server.
...
from ragmonk.core.errors import ConfigError, UsageError
...
def serve(
    ...
    if not mcp:
        raise UsageError(
            "ragmonk serve currently requires --mcp "
    ...
    with AppContext.bootstrap() as ctx:
        if not ctx.config.mcp.enabled:
            raise ConfigError(
                ...
                "run `ragmonk config set mcp.enabled true` to enable it."
            )
    ...
    # has to go to stderr.
    error_console.print("[bold]RagMonk MCP server starting on stdio...[/bold]")
    # Deferred import (CLI performance improvement plan, Phase 2): the mcp
    # SDK must only load for `ragmonk serve --mcp`, never for any other
    # command that happens to import this module.
    from ragmonk.mcp.server import run_stdio
