"""Root Typer application wiring every RagMonk subcommand."""
...
app.command("serve", help="Start RagMonk as a server (MCP over stdio).")(serve.serve)
...
app.command("backup", help="Create a restorable backup archive of RagMonk's databases.")(
    backup.backup
)
app.command("restore", help="Restore RagMonk's databases from a backup archive.")(
    restore.restore
)
...
app.command(
    "uninstall", help="Remove the RagMonk application and, by default, all of its data."
)(uninstall.uninstall)
