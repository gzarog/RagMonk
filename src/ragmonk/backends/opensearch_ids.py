"""Deterministic ``_id`` derivation for the OpenSearch adapter.

Storage backend abstraction plan, Phase 4: OpenSearch must never be left
to auto-generate ``_id`` for a document derived from RagMonk's own
...
This module has no OpenSearch import and no ``opensearch-py`` dependency
-- it is pure string/hash logic, safe to import unconditionally.
"""
...
def generation_marker_id(source_id: str) -> str:
    ...
    generation marker (see ``opensearch.py``'s generation lifecycle
