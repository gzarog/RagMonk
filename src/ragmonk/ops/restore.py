"""``ragmonk restore``: reinstate a ``ragmonk backup`` archive as the
live runtime state.
...
from ragmonk.core.errors import DatabaseError, UsageError
from ragmonk.ops.backup import MANIFEST_FORMAT_VERSION
from ragmonk.service import pid
from ragmonk.storage.migrations import MIGRATIONS, DatabaseKind
...
def _extract_archive(archive: Path, staging: Path) -> None:
    try:
        with tarfile.open(archive, "r:gz") as tar:
            ...
            # from anywhere other than this machine's own ``ragmonk
            # backup``.
...
def restore_backup(archive: Path, *, home: Path, restart_daemon: bool = True) -> RestoreReport:
    ...
    try:
        ...
        if not isinstance(format_version, int) or format_version > MANIFEST_FORMAT_VERSION:
            raise DatabaseError(
                f"backup archive format v{format_version} is newer than this RagMonk "
        ...
        if sources_version > latest_sources:
            raise DatabaseError(
                f"sources.db schema v{sources_version} is newer than this RagMonk "
            ...
            for project_dir in sorted(staged_projects_dir.iterdir()):
                ...
                if version > latest_knowledge:
                    raise DatabaseError(
                        ...
                        f"RagMonk version understands (v{latest_knowledge}); refusing to restore"
                    )
        ...
        if daemon_was_running and restart_daemon:
            from ragmonk.cli import daemon as daemon_cli
