"""``ragmonk uninstall``: removes the installed RagMonk application
...
1. **Purge data** (default; skipped with ``--keep-data``, see
   ...
   ``RAGMONK_HOME`` entirely (databases, config, backups, logs,
...
2. **Remove the application itself**, dispatched by installation method
   ...
   installed" question ``ragmonk update install`` already answers). An
...
share a parent directory with ``RAGMONK_HOME``, and never anything else
...
from ragmonk.update.installer import (
    CommandRunner,
    InstallMethod,
...
def _launcher_name() -> str:
    return "ragmonk.cmd" if sys.platform == "win32" else "ragmonk"


def _manual_instructions(method: InstallMethod) -> str:
    if method is InstallMethod.EDITABLE:
        return (
            "this is an editable/dev RagMonk install (`pip install -e .`); remove it by "
            "deleting your source checkout (and optionally `pip uninstall ragmonk`)."
        )
    return (
        "could not determine how RagMonk was installed; remove it manually, e.g. "
        "`pip uninstall ragmonk` or `pipx uninstall ragmonk`, or delete its install "
        "directory."
    )
...
def remove_application(plan: UninstallPlan, *, runner: CommandRunner | None = None) -> bool:
    ...
    if plan.method is InstallMethod.PIP:
        return run([sys.executable, "-m", "pip", "uninstall", "-y", "ragmonk"], None) == 0
    if plan.method is InstallMethod.PIPX:
        return run(["pipx", "uninstall", "ragmonk"], None) == 0
...
def purge_home(home: Path) -> bool:
    ...
    if not home.exists():
        return False
    pid.stop_and_wait(home, action="delete this data", error_cls=RagMonkError)
