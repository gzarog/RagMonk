...
``python -m ragmonk.update.background <home>`` process that does the one
...
explicit ``ragmonk update check`` reports a failure.
"""
...
def maybe_launch_background_check(home: Path, config: UpdatesConfig) -> None:
    ...
    with contextlib.suppress(OSError):
        subprocess.Popen(  # noqa: S603 - fixed argv (sys.executable + literal module name), no shell
            [sys.executable, "-m", "ragmonk.update.background", str(home)],
