...
(``ragmonk serve --mcp``) the MCP stdio transport's JSON-RPC channel
...
def maybe_notify(home: Path, config: UpdatesConfig, *, console: Console | None = None) -> None:
    ...
    out.print(
        f"\n[bold]A newer RagMonk version is available:[/bold] "
        f"{installed} → {cached.latest_version}"
    )
    out.print("Run `ragmonk update install` to upgrade.")
