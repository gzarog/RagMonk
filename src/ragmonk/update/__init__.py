...
Distinct from ``ragmonk upgrade``/``ops/upgrade.py``, which applies
database *schema* migrations to an already-installed RagMonk -- this
package is about discovering and installing a newer RagMonk *release*
itself. See ``cli/update.py`` for the ``ragmonk update ...`` commands and
...
- ``cache``: reads/writes ``<RAGMONK_HOME>/update.json`` so a later
...
- ``installer``: installing a newer release once found -- fleshed out in
  ...
  check); ``ragmonk update install`` is a clearly-labeled stub until then.
"""
