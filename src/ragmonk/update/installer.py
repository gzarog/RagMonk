"""Installing a newer RagMonk release (CLI performance improvement plan,
...
sequence ``ragmonk update install`` (``cli/update.py``) runs.
...
it's a real semantic version of ``gzarog/RagMonk``'s own releases --
...
_DISTRIBUTION_NAME = "ragmonk"

_INSTALL_SH_URL = "https://raw.githubusercontent.com/gzarog/RagMonk/main/install.sh"
_INSTALL_PS1_URL = "https://raw.githubusercontent.com/gzarog/RagMonk/main/install.ps1"
_GIT_URL = "https://github.com/gzarog/RagMonk.git"
...
class UpdateInstallError(Exception):
    """This RagMonk install can't be upgraded automatically (an
...
class InstallMetadata:
    """``<RAGMONK_HOME>/install_info.json``'s schema -- written by
...
def detect_install_method(home: Path) -> InstallMethod:
    ...
    else "unknown" (no ``ragmonk`` distribution metadata at all -- e.g.
...
def _upgrade_invocation(method: InstallMethod, tag: str) -> tuple[list[str], dict[str, str] | None]:
    ...
    if method is InstallMethod.INSTALL_SCRIPT:
        ...
        return (["sh", "-c", f"curl -fsSL {_INSTALL_SH_URL} | sh"], {"RAGMONK_REF": tag})
...
def _manual_instructions(method: InstallMethod) -> str:
    if method is InstallMethod.EDITABLE:
        return (
            "this is an editable/dev RagMonk install (`pip install -e .`); "
            "upgrade it with `git pull` in the source checkout instead."
        )
    return (
        "could not determine how this RagMonk was installed. Upgrade manually:\n\n"
...
def install_latest(home: Path, *, runner: CommandRunner | None = None) -> InstallOutcome:
    ...
    modules -- run schema migrations (``ragmonk upgrade``) and a health
    check (``ragmonk doctor``), reusing those existing commands rather
    than duplicating their logic.
    """
    from ragmonk.update import checker, versioning
    ...
    migrations_applied = run([sys.executable, "-m", "ragmonk.cli.main", "upgrade"], None) == 0
    healthy = run([sys.executable, "-m", "ragmonk.cli.main", "doctor"], None) == 0
