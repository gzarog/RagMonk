"""Resolution of RagMonk's on-disk runtime layout.

``RAGMONK_HOME`` always wins so tests (and power users) can redirect the
...
def runtime_dir() -> Path:
    override = os.environ.get("RAGMONK_HOME")
    if override:
        return Path(override).expanduser()
    if sys.platform == "win32":
        base = os.environ.get("LOCALAPPDATA")
        if base:
            return Path(base) / "RagMonk"
        return Path.home() / "AppData" / "Local" / "RagMonk"
    return Path.home() / ".ragmonk"
...
def project_config_path(cwd: Path | None = None) -> Path:
    return (cwd or Path.cwd()) / ".ragmonk.yaml"
...
def daemon_pid_path(home: Path | None = None) -> Path:
    """The Phase 7 daemon's PID-file -- one daemon per ``RAGMONK_HOME``,
...
def daemon_health_path(home: Path | None = None) -> Path:
    ...
    so ``ragmonk daemon status``/``doctor`` can read it from a fresh CLI
...
def update_cache_path(home: Path | None = None) -> Path:
    ...
    ``ragmonk update check`` writes and ``ragmonk update status`` reads
    -- one per ``RAGMONK_HOME``, mirroring ``daemon_health_path`` above.
